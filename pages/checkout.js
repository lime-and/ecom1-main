        ...
        <div className="flex mb-5 items-center" key={productInfo._id}>
          ...
          <div className="pl-4 items-center">
            ...
            <div className="flex mt-1">
              <div className="grow font-bold">円{productInfo.price}</div>
              <div>
                <button onClick={() => lessOfThisProduct(productInfo._id)} className="border border-yellow-400 px-2 rounded-lg text-yellow-400">-</button>
                <span className="px-2">
                  {selectedProducts.filter(id => id === productInfo._id).length}
                </span>
                <button onClick={() => moreOfThisProduct(productInfo._id)} className="bg-yellow-400px-2 rounded-lg text-white">+</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <form action="/api/checkout" method="POST">
        ...
        <div className="mt-8">
          <div className="flex my-3">
            ...
            <h3 className="font-bold">{subtotal}円</h3>
          </div>
          <div className="flex my-3">
            <h3 className="grow font-bold text-gray-400">Delivery:</h3>
            <h3 className="font-bold">{deliveryPrice}円</h3>
          </div>
          <div className="flex my-3 border-t pt-3 border-dashed border-yellow-400">
            <h3 className="grow font-bold text-gray-400">Total:</h3>
            <h3 className="font-bold">{total}円</h3>
          </div>
        </div>
        <input type="hidden" name="products" value={selectedProducts.join(',')}/>
        <button type="submit" className="bg-yellow-400 px-5 py-2 rounded-xl font-bold text-white w-full my-4 shadow-yellow-400 shadow-lg">Pay 円{total}</button>
      </form>
    </Layout>
  );
}
